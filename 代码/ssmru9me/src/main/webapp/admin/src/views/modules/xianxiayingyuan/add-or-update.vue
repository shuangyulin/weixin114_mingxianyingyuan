    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="参与人数" prop="canyurenshu">
          <el-input v-model="ruleForm.canyurenshu" 
              placeholder="参与人数" clearable  :readonly="ro.canyurenshu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="参与人数" prop="canyurenshu">
              <el-input v-model="ruleForm.canyurenshu" 
                placeholder="参与人数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.huodongxiangqing = this.ruleForm.huodongxiangqing.replace(reg,'../../../ssmru9me/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xianxiayingyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xianxiayingyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
