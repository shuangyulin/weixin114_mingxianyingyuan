...
export default {
  ...
  methods: {
	...
	this.ruleForm.wupinxiangqing = this.ruleForm.wupinxiangqing.replace(reg,'../../../ssmru9me/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yingyuanwupin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yingyuanwupinCrossAddOrUpdateFlag = false;
                  this.parent.search();
